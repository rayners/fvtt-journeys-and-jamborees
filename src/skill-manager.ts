...
export class SkillManager {
  ...
  getAvailableSkills(): Record<string, string> {
    ...
    const worldSkillItems = game.items.filter((i: Item) => i.type === 'skill');
    ...
    const sampleActor = game.actors.find(
      (a: Actor) => a.type === 'character' || a.type === 'pc' || a.type === 'player'
    );
    ...
    if (sampleActor?.system?.attributes) {
      Object.entries(sampleActor.system.attributes).forEach(([key, attr]: [string, any]) => {
        if (
          attr &&
          typeof attr === 'object' &&
          (attr.value !== undefined || attr.rank !== undefined)
        ) {
    ...
    if (Object.keys(skills).length === 0) {
      skills['configure-me'] =
        game.i18n.localize('J&J.skills.configureMe') || 'Please Configure Skills';
    }
  ...
  private async getSkillsFromCompendiums(): Promise<Record<string, string>> {
    ...
    const itemCompendiums = game.packs.filter(
      (pack: CompendiumCollection) => pack.documentName === 'Item'
    );
    ...
    for (const pack of itemCompendiums) {
      try {
        ...
        const skillEntries = index.filter(
          (entry: any) =>
            entry.type === 'skill' ||
            entry.name?.toLowerCase().includes('skill') ||
            // Some systems might use different naming
            metadata.id.includes('skill') ||
            metadata.label?.toLowerCase().includes('skill')
        );
  ...
  private getSkillsFromCompendiums(): Record<string, string> {
    ...
    const systemModules = game.modules.filter(
      (m: any) =>
        m.active &&
        (m.id.includes(game.system.id) ||
          m.relationships?.systems?.some((s: any) => s.id === game.system.id))
    );
  ...
  validateConfiguredSkills(): { pathfinding: boolean; lookout: boolean; quartermaster: boolean } {
    ...
    return {
      pathfinding: this.isSkillValid(
        game.settings.get('journeys-and-jamborees', 'pathfinderSkillName'),
        availableSkills
      ),
      lookout: this.isSkillValid(
        game.settings.get('journeys-and-jamborees', 'lookoutSkillName'),
        availableSkills
      ),
      quartermaster: this.isSkillValid(
