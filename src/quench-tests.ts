    ...
    describe('Party Actor Integration Tests', function() {
      ...
      describe('Travel Roles', function() {
        it('should assign pathfinder role', async function() {
          ...
          try {
            ...
            assert.equal(partyActor.system.roles.pathfinder, character.id, 
        ...
        it('should clear role when character is removed', async function() {
          ...
          try {
            ...
            assert.notOk(partyActor.system.roles.pathfinder, 
              'Pathfinder role should be cleared');
          } finally {
            await character.delete();
          }
        });
      ...
      describe('Resource Management', function() {
        it('should add resources', async function() {
          ...
          assert.equal(partyActor.system.resources.rations, initialRations + 5,
        ...
        it('should remove resources', async function() {
          ...
          assert.equal(partyActor.system.resources.rations, 7,
            'Rations should be reduced to 7');
        });
        ...
        it('should not reduce resources below zero', async function() {
          ...
          assert.equal(partyActor.system.resources.water, 0,
            'Water should not go below 0');
        });
      });
    });
  });
    ...
    describe('Party Sheet UI Tests', function() {
      ...
      it('should display member count', async function() {
        ...
        try {
          ...
          assert.ok(memberCount.text().includes('2'), `Should show 2 members, but shows: ${memberCount.text()}`);
        } finally {
          await char1.delete();
          await char2.delete();
        }
    ...
    quench.registerBatch('journeys-and-jamborees.system-dragonbane', (context) => {
      ...
      describe('Dragonbane System Tests', function() {
        ...
        it('should use Dragonbane skill names', async function() {
          ...
          assert.equal(systemConfig.skills.pathfinding, 'bushcraft', 'Default pathfinding skill should be bushcraft');
          assert.equal(systemConfig.skills.lookout, 'awareness', 'Default lookout skill should be awareness');
          assert.equal(systemConfig.skills.quartermaster, 'bartering', 'Default quartermaster skill should be bartering');
          
          // Check that the settings exist and have reasonable defaults
          const pathfinderValue = game.settings.get('journeys-and-jamborees', 'pathfinderSkillName');
        ...
        it('should roll pathfinding with Dragonbane dice', async function() {
          ...
          const pathfinderSkill = game.settings.get('journeys-and-jamborees', 'pathfinderSkillName');
    ...
    quench.registerBatch('journeys-and-jamborees.system-dnd5e', (context) => {
      ...
      describe('D&D 5e System Tests', function() {
        ...
        it('should use D&D 5e movement rates', function() {
          assert.equal(partyActor.system.movement.value, 24, 'On-foot movement should be 24 miles/day');
          assert.equal(partyActor.system.settings.baseMovement, 24, 'Base movement should be 24 miles');
        });
    ...
    quench.registerBatch('journeys-and-jamborees.system-generic', (context) => {
      ...
      describe('Generic System Tests', function() {
        ...
        it('should use generic defaults', function() {
          assert.equal(partyActor.system.movement.value, 25, 'Should use default 25 units movement');
          assert.equal(partyActor.system.settings.baseMovement, 25, 'Base movement should be 25 units');
        });
