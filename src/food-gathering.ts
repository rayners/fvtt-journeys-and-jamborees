...
export class FoodGatheringSystem {
  ...
  private async rollSkillWithTracking(
    ...
    const timeoutSeconds =
      game.settings.get('journeys-and-jamborees', 'foodGatheringRollTimeout') || 30;
    const timeoutMs = timeoutSeconds * 1000;

    // Notify the user that we're waiting for their roll
    ui.notifications.info(
      game.i18n.format('J&J.FoodGathering.WaitingForRoll', {
        skill: skillName,
   ...
   */
  private checkRangedWeapon(actor: Actor): {
  ...
  private checkHuntingTrap(actor: Actor): boolean {
    ...
    return actor.items.some(
      item =>
        item.type === 'item' &&
        (item.name.toLowerCase().includes('trap') || item.name.toLowerCase().includes('snare'))
    );
  }
  ...
  async hunt(actor: Actor): Promise<HuntingResult> {
    ...
    if (
      !weaponCheck.hasRangedWeapon &&
      animalResult.requiresWeapon &&
      (!hasTrap || !animalResult.canUseTrap)
    ) {
      return {
        ...animalResult,
        success: false,
  ...
  async fish(actor: Actor, hasRod: boolean, hasNet: boolean): Promise<FoodGatheringResult> {
    ...
    return {
      ...
      description: game.i18n.format('J&J.FoodGathering.FishCaught', {
        rations: rationsRoll.total,
        gear: hasNet
          ? game.i18n.localize('J&J.FoodGathering.FishingNet')
          : game.i18n.localize('J&J.FoodGathering.FishingRod')
      })
    };
  }
  ...
  async forage(
    ...
    const foragingSkill = game.settings.get(
  ...
  async cook(
    actor: Actor,
    rawRations: number,
    hasFieldKitchen: boolean = false,
    hasProperKitchen: boolean = false
  ): Promise<FoodGatheringResult> {
    if (!this.isDragonbaneSystem() || !this.isDragonbaneCoresetAvailable()) {
      return {
        success: false,
    ...
    return {
      ...
      description: game.i18n.format('J&J.FoodGathering.CookingSuccess', {
        rations: rawRations,
        kitchen: hasProperKitchen
          ? game.i18n.localize('J&J.FoodGathering.ProperKitchen')
          : hasFieldKitchen
            ? game.i18n.localize('J&J.FoodGathering.FieldKitchen')
